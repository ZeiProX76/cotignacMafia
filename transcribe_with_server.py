#!/usr/bin/env python3
"""
Transcribe local videos by serving them via local HTTP server.
"""
...
def transcribe_video(video_path: str, output_name: str, server_port: int, verbose: bool = True):
    """Transcribe a local video file via HTTP server."""
    ...
    # Create HTTP URL
    video_url = f"http://localhost:{server_port}/{video_path.name}"
    ...
    try:
        ...
        print(f"\n❌ Error transcribing {video_path.name}: {e}")
        raise


if __name__ == "__main__":
    try:
        # Start HTTP server to serve the Downloads directory
        downloads_dir = "/home/hugues/Downloads"
        port = 8765

        print("\n🌐 Starting local HTTP server...")
        print(f"   Directory: {downloads_dir}")
        print(f"   Port: {port}")
        httpd, port = start_http_server(downloads_dir, port)
        ...
        result1 = transcribe_video(
            "/home/hugues/Downloads/get (1).mp4",
        ...
        result2 = transcribe_video(
            "/home/hugues/Downloads/get (2).mp4",
